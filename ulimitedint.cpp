...
int is_greater(int size1, int size2, int *arr1, int *arr2){
    ...
    if (size1 > size2) return 1;
        ...
        for (int i = size1 - 1; i >= 0; i--){
            ...
            else if (arr1[i] < arr2[i]) return 2;
        }
    }
    return 0;
}
...
bool IsZero1(UnlimitedInt* z){
    if(z->get_array()[z->get_size()-1]!=0) return false;
    return true;
}
...
UnlimitedInt* abs(UnlimitedInt* i1){
    UnlimitedInt* absValue = new UnlimitedInt(i1->get_array(), i1->get_size(), 1, i1->get_size());
...
UnlimitedInt* UnlimitedInt::add(UnlimitedInt* i1, UnlimitedInt* i2){
    ...
    for(int i = 0; i<result->size; i++){
        int digit1, digit2;
        digit1 = (i<i1->size) ? i1->unlimited_int[i] : 0;
        digit2 = (i<i2->size) ? i2->unlimited_int[i] : 0;
...
//     UnlimitedInt* b= i1.mul(&i1, &i2);
//     UnlimitedInt* c= i1.add(&i1, &i2);
//     UnlimitedInt* d= i1.sub(&i1, &i2);
//     UnlimitedInt* e= i1.mod(&i1, &i2);
